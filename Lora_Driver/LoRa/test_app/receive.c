...
int main(int argc, char **argv)
{
	...
	set_state(fd, LORA_STATE_STANDBY);
	...
	printf("001--The LoRa STATE  %X LENGTH \n", get_state(fd));
  */
	...
	//for(;;)
	{
		...
		//printf("002--The LoRa STATE  %X LENGTH \n", get_state(fd));
		...
		if (len > 0) 
		{
         ...
         else
         {
          ...
          printf("%uLPKT RSSI\n", get_LPKTRSSI(fd));
          printf("%d SNR\n",get_snr(fd));
          printf("%c%c-%c%c|%f",buffer[4],buffer[5],buffer[2],buffer[3], temprature);
         }
        }
			}
			for (i = 0; i < len; i++)
