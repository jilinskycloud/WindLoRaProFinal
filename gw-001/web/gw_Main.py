...
def console_logs():
    if 'username' in session:
        ...
        _dist = subprocess.Popen("cat /tmp/readDist_daemon.log", shell=True, stdout=subprocess.PIPE).stdout
        _dist =  _dist.read()
        ...
        return render_template('console-logs.html', data=pc, flask_log=flask_log, hb_log=hb_log, _postD_log=_postD_log, _reb_log=_reb_log, _pro_log=_pro_log, _temp_log=_temp_log, _dist_log=_dist_log, _tempR_log=_tempR_log, _distR_log=_distR_log)
    else:
        return redirect(url_for('login'))
###
