...
def sendLogs():
	...
	log("Send Log File to the server...")
	form = MultiPartForm()
	# Add a fake file
		...
		try:
			if HB == "ON":
				for SN in r_sensor_conf:
					...
					#print("Heart-Beat Daemon ==> DATA::",rec_pkt['data'])
					if rec_pkt['msg'] == 'reboot':
						...
						pid_1 = pi1.read()
						#print("Config from Server is receives", rec_pkt['msg'])
						...
						with open(gw_conf,'w') as f:
							...
							log("New Sensor Config is written to the file")
						os.system("reboot")
						...
						#print("Log Request from server received ::", rec_pkt['msg'])
					elif rec_pkt['msg'] == 'del_SnId':
						#print("delete SnId from server is :: ", rec_pkt['data'])
